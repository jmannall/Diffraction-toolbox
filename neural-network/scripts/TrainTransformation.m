function [net, epochLosses, losses] = TrainTransformation(net, trainingData, targetData, numEpochs, miniBatchSize, numIterationsPerEpoch, lossFunc, dataFunc)
    ...
    for epoch = i:numEpochs
        ...
        for i = 1:numIterationsPerEpoch
            ...
            [loss, gradients] = dlfeval(lossFunc,net,psi,psiInv,X,T);
            ...
            for k = 1:size(gradients, 1)
                g = extractdata(gradients(k,:));
                gNorm = norm(g);
                if gNorm > maxGradient
                    gradients(k,:) = g * maxGradient ./ gNorm;
                end
            end

            gradients = max(-maxGradient, min(gradients, maxGradient));
            [psi,averageGrad,averageSqGrad] = adamupdate(psi,gradients,averageGrad,averageSqGrad,iteration,learnRate,gradDecay,sqGradDecay);
            
            psiInv = dlarray(pinv(extractdata(psi)));
